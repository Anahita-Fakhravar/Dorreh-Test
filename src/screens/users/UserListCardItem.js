...
export const UserListCardItem = ({ item, index, y }) => {
    ...
    const position = Animated.subtract(index * responsiveHeight(18), y);
    const isDisappearing = -responsiveHeight(18);
    const isTop = 0;
    const isBottom = responsiveHeight(100) - responsiveHeight(18);
    const isAppearing = responsiveHeight(100);

    const translateY = Animated.add(y, y.interpolate({
        inputRange: [0, index * responsiveHeight(18)],
        outputRange: [0, -index * responsiveHeight(18)],
